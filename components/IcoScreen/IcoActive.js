...
import { getApi, pushFirebase } from '../../services/getApi';
...
class IcoActive extends Component {
    constructor(props) {
        super(props)
        this.state = {
            isLoading: true,
    ...
    getApiData = () => {
        fetch("http://chasing-coins.com/api/v1/icos/active")
        .then((res) => res.json())
            .then((icoApi) => {
        ...
        if (this.props.userAuth !== null) {
            ...
            if (search === undefined) {
                pushFirebase("icoLikes/" + this.props.userAuth._user.uid, item).then((data) => {
                    if (data === null) {
                        this.getLikeIco(this.state.apiIcoActive);
                    }
                })
            }
        } else {
        ...
        return (
            <Container>
                <Header androidStatusBarColor="#D8D8D8" style={{backgroundColor: "#D8D8D8"}}>
                    ...
                    <Body >
                        <Title style={{color: "#000000"}}>ACTIVE</Title>
                    </Body>
                    <Right />
                </Header>
                <FlatList
                    data={this.state.apiIcoActive}
...
export default connect(mapStateToProps)(IcoActive);